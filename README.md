...
5. Gemini가 추출 본문 또는 provider 요약을 한국어로 요약하고 금리·환율·반도체·실적·전쟁·지정학 등의
   카테고리로 분류합니다.
6. 미국·국내 지수, 반도체, 미국 10년물 금리, 원·달러 환율과 최근 주요 뉴스의 신호 강도를
   비교해 오늘 확인할 3가지를 자동 선정하고 수치와 근거 기사를 보여줍니다. 뉴스 범위는
   평일 36시간이며 월요일에는 주말을 포함해 72시간입니다.
...
| 오늘 확인할 3가지 | 시장 변동폭과 뉴스 중요도를 비교해 상위 3개 동적 선정 | 토스증권·FRED·저장 뉴스 |
...
| 국장·미장 포트폴리오 | 실제 보유 종목, 현재가, 평균단가, 손익 및 시장별 비중 | 토스증권 Open API |
| 계좌 연동 | 계좌 자동 선택, 보유 종목 조회, 빈 계좌·데모 전환 | 토스증권 Open API |
| 배치 처리 | 07:10 뉴스 수집 동작, 07:30 브리핑 작업은 placeholder | Celery Beat·Redis |

실제 provider를 설정하면 관심 종목 가격, 포트폴리오, 국내외 시장 지수, 미국 10년물 금리,
USD/KRW, 뉴스와 오늘의 포커스는 외부 데이터를 사용합니다. 보유 종목이 없는 계좌는 정상적인
빈 상태로 표시하며, 화면의 `데모 보기` 버튼으로 샘플 국장·미장 포트폴리오를 확인할 수 있습니다.
...
Browser
  ...
  │    ├─ Toss: 계좌·보유 종목, 반도체 종목 현재가, USD/KRW, KOSPI·KOSDAQ
...
후 `GET /api/v1/broker/accounts`에서 조회할 수 있습니다. 비워두면 조회된 첫 번째 계좌를 자동
선택합니다. 보유 자산 조회에는 선택한 계좌가 `X-Tossinvest-Account` 헤더로 전달됩니다.
보유 종목이 없으면 오류가 아니라 연결된 빈 포트폴리오로 표시됩니다.
...
| GET | `/api/v1/broker/holdings` | 선택 계좌의 실제 국내·미국 보유 종목과 손익 조회 |
